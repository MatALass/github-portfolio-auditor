...
def render_overview(data: DashboardData) -> None:
    ...
    with row_one[0]:
        _metric_card(
            "Repositories analyzed", str(metrics["total_repositories"]), "Current audited inventory"
        )
    with row_one[1]:
        _metric_card(
            "Raw portfolio median",
            f"{metrics['median_score']:.2f}/100",
            "Median across all repositories, including weak archives.",
        )
    with row_one[2]:
        _metric_card(
            "Highlight now",
            str(metrics["highlight_count"]),
            "Repositories already strong enough to feature directly.",
        )
    with row_one[3]:
        _metric_card(
            "Redundancy clusters",
            str(metrics["redundancy_clusters"]),
            "Overlap clusters detected from redundancy analysis",
        )
    ...
    with row_two[0]:
        _metric_card(
            "Keep visible",
            str(metrics["keep_visible_count"]),
            "Public repositories worth keeping while improving them.",
        )
    with row_two[1]:
        _metric_card(
            "Improve backlog",
            str(metrics["improvement_backlog_count"]),
            "Repos with upside but not strong enough to represent the portfolio yet.",
        )
    with row_two[2]:
        _metric_card(
            "Discard candidates",
            str(metrics["discard_count"]),
            "Repos that should leave the public spotlight in current state.",
        )
    with row_two[3]:
        _metric_card(
            "Selected scope avg",
            f"{metrics['selected_scope_average']:.2f}/100",
            "Average across keep + improve scope.",
        )
    ...
    if optimizer["top_actions"]:
        ...
        for index, (column, action) in enumerate(
            zip(action_cols, optimizer["top_actions"], strict=False), start=1
        ):
            with column:
