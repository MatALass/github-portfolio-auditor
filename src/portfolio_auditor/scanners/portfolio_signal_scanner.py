...
class PortfolioSignalScanner(BaseScanner):
    ...
    def scan(self, repo_path: Path) -> RepoScanSection:
        ...
        readme_text = (
            readme.read_text(encoding="utf-8", errors="ignore").lower() if readme.exists() else ""
        )

        signals = {
            "has_demo_language": any(
                word in readme_text
                for word in ["demo", "dashboard", "cli", "model", "pipeline", "architecture"]
            ),
        ...
        if signals["name_quality"] == "weak":
            issues.append(
                Issue(
                    code="WEAK_REPO_NAME",
                    severity="medium",
                    message="Repository name is too generic for strong portfolio signaling.",
                )
            )
